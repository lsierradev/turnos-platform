...
export function tallerDelRequest(
  ...
  if (user.rol === Rol.ADMIN || user.rol === Rol.TECNICO)
    return user.taller ?? null;
  const valor = Array.isArray(encabezado) ? encabezado[0] : encabezado;
  if (valor === undefined || valor === "") return null;
  if (!esUuid(valor))
    throw new BadRequestException("X-Taller no es un id valido.");
  return valor;
}
...
export class ContextoInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    return from(
      this.db.ejecutarComo(
        { usuarioId: user.sub, rol: user.rol, tallerId },
